...
在 24 种通用 AI 模式检测之外，本 skill 新增如下硬约束：

1. **引文源语言**：引文必须使用著作原始语言，不得擅自翻译。翻译即损耗，源语言承载不可替代的概念精确性。
2. **全书论证结构**：当引用整部著作时，必须描述该书的论证推进结构（推进类型/关键转折点/结构评价），不得仅概括内容。

以上两条纳入质量评分：新增「引文源语言」和「全书论证结构」两个评分维度，总分从 50 调整为 70。
