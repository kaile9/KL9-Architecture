...
| 模块 | 导入时的行为 |
|------|-------------|
| `graph_backend.py` | bridge.py 通过 store_concept() + create_edge() 将导入概念写入 SQLite；新增概念自动参与 BM25 检索和张力共振排序 |
| `memory.py` | 导入操作记录为 session；通过 retrieve_by_tension_context() 可查到导入历史 |
| `tension_bus.py` | 导入完成后发射 SkillbookImportEvent，订阅者可响应（如 learner 触发重评估） |
| `core_structures.py` | 导入的 DualState 数据直接映射到 Perspective + Tension 结构 |
