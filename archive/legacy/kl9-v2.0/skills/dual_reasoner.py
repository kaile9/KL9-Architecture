"""
KL9-RHIZOME v2.0 · Dual-Reasoner
...
from core.structures import Perspective, Tension, PerspectiveType
from core.tension_bus import TensionBus2, bus, CompressionTensionEvent, DualPerspectiveEvent, TensionSubscription
...
class DualReasoner:
    ...
    核心功能：
    1. 构建 Perspective A（理论视角）
    2. 构建 Perspective B（具身视角）
    3. 生成 A/B 之间的张力
    4. 发射到 TensionBus
    """
    ...
    def on_compression_event(self, event: CompressionTensionEvent):
        """
        响应压缩事件，构建 A/B 视角
        ...
        # 4. 发射到 TensionBus
        self.bus.emit(DualPerspectiveEvent(
    ...
    def _construct_theoretical_perspective(self, query: str) -> Perspective:
        """
        构建理论视角（Perspective A）
    ...
    def _construct_embodied_perspective(self, query: str) -> Perspective:
        """
        构建具身视角（Perspective B）
    ...
    def _generate_tension(self, A: Perspective, B: Perspective, 
                          query: str) -> Tension:
        """
        生成 A/B 之间的张力
    ...
    def _find_irreconcilable_points(self, A: Perspective, B: Perspective, 
                                    query: str) -> List[str]:
        """
        识别不可调和点
    ...
    def _extract_claim(self, perspective: Perspective, query: str) -> str:
        """
        提取视角的主张
    ...
    def _infer_tension_type(self, A: Perspective, B: Perspective, 
                            query: str) -> str:
        """
        推断张力类型
        ...
        if A.perspective_type == PerspectiveType.THEORETICAL and \
           B.perspective_type == PerspectiveType.EMBODIED:
            return "dialectical"
