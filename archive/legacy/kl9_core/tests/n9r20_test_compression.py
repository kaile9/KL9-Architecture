"""
9R-2.0 RHIZOME · N9R20CompressionCore 单元测试

测试范围：
1. N9R20AdaptiveFourModeCodec — 四模编码器模式序列选择
2. N9R20SemanticValidator — 语义验证器
3. N9R20DynamicFoldEngine — 动态折叠引擎
4. N9R20CompressionCore — 完整压缩流程
5. 压缩率硬性要求 2.0-2.5x
6. fold_depth 动态 2-9
7. 四模编码完整流程
8. N9R20DualState 构建与转换
9. N9R20TensionBus 集成
10. 边界条件与极端输入
"""
...
from skills.n9r20_compression_core import (
...
class TestN9R20AdaptiveFourModeCodec(unittest.TestCase):
    """四模编码器测试"""

    def setUp(self):
        self.codec = N9R20AdaptiveFourModeCodec()
...
class TestN9R20SemanticValidator(unittest.TestCase):
    """语义验证器测试"""
    ...
    def test_validation_returns_result(self):
        ...
        self.assertIsInstance(result, N9R20ValidationResult)
...
class TestN9R20DynamicFoldEngine(unittest.TestCase):
    """动态折叠引擎测试"""

    def setUp(self):
        self.engine = N9R20DynamicFoldEngine()
...
# ═══════════════════════════════════════════════════════════
# N9R20CompressionCore — 完整压缩流程
# ═══════════════════════════════════════════════════════════

class TestN9R20CompressionCoreFullPipeline(unittest.TestCase):
    """完整压缩流程测试"""

    def setUp(self):
        self.core = N9R20CompressionCore()

    def test_compress_returns_compressed_output(self):
        """compress() 返回 N9R20CompressedOutput"""
        result = self.core.compress(
            query="量子纠缠与缘起性空的比较研究",
            session_id="test-session"
        )
        self.assertIsInstance(result, N9R20CompressedOutput)
...
class TestN9R20CompressionCoreRatioConstraint(unittest.TestCase):
    """压缩率硬性要求 2.0-2.5x"""
    ...
    def test_target_ratio_never_exceeds_2_5(self):
        ...
        for length in [10, 100, 500, 1000]:
            query = "A" * length
            routing = self.core.n9r20_router.route(query)
...
class TestN9R20CompressionCoreFoldDepthConstraint(unittest.TestCase):
    """fold_depth 动态 2-9"""
...
# ═══════════════════════════════════════════════════════════
# N9R20DualState 构建与转换
# ═══════════════════════════════════════════════════════════

class TestN9R20DualStateConstruction(unittest.TestCase):
    """N9R20DualState 构建测试"""
    ...
    def test_build_initial_state(self):
        ...
        self.assertIsInstance(state, N9R20DualState)
    ...
    def test_initial_state_has_both_perspectives(self):
        ...
        self.assertIsInstance(state.perspective_A, N9R20Perspective)
        self.assertIsInstance(state.perspective_B, N9R20Perspective)
    ...
    def test_initial_state_has_tension(self):
        ...
        self.assertIsInstance(state.tension, N9R20Tension)
...
class TestFourModeFolding(unittest.TestCase):
    ...
    def test_expand_semantic_space_preserves_query(self):
        """展开语义空间保留原始 query"""
        state = N9R20DualState(query="原始查询", compressed_output="")
...
# ═══════════════════════════════════════════════════════════
# N9R20CompressionCore — 边界条件
# ═══════════════════════════════════════════════════════════

class TestN9R20CompressionCoreEdgeCases(unittest.TestCase):
    """边界条件与极端输入"""
...
class TestN9R20CompressionCoreIntegrity(unittest.TestCase):
    """压缩完整性验证"""
    ...
    def test_multiple_compressions_independent(self):
        ...
        # session_id 向 N9R20CompressedOutput 传播
...
class TestN9R20CompressionCoreHelpers(unittest.TestCase):
    """辅助函数测试"""
...
class TestN9R20CompressionCoreSingleton(unittest.TestCase):
    """全局单例测试"""

    def test_n9r20_compression_core_singleton_exists(self):
        """n9r20_compression_core 全局单例存在"""
        self.assertIsNotNone(n9r20_compression_core)
        self.assertIsInstance(n9r20_compression_core, N9R20CompressionCore)

    def test_singleton_has_all_components(self):
        """单例包含所有组件"""
        self.assertIsNotNone(n9r20_compression_core.four_mode_codec)
        self.assertIsNotNone(n9r20_compression_core.fold_engine)
        self.assertIsNotNone(n9r20_compression_core.validator)
        self.assertIsNotNone(n9r20_compression_core.n9r20_bus)
        self.assertIsNotNone(n9r20_compression_core.n9r20_router)

    def test_singleton_compress_works(self):
        """单例压缩功能正常"""
        result = n9r20_compression_core.compress(
            query="测试全局单例",
...
class TestN9R20CompressionCoreStress(unittest.TestCase):
    """压力测试"""
