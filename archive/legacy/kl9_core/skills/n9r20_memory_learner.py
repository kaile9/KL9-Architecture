...
class N9R20MemoryLearner:
    ...
    def __init__(self, max_memories: int = 1000, forget_halflife: float = 86400.0):
        """
        Args:
            max_memories: 最大记忆数量（环形缓冲区）
            forget_halflife: 遗忘半衰期（秒），默认 24 小时
        """
        # ═══ 记忆存储 ═══
        self._memories: OrderedDict[str, N9R20SessionMemory] = OrderedDict()
        self._max_memories = max_memories
        self._forget_halflife = forget_halflife
        self._forget_constant = math.log(2) / forget_halflife
        ...
        # ═══ 传播配置 ═══
        self._propagation_rate: float = 0.1   # 技能传播率
        self._propagation_threshold: float = 0.15  # 性能差异阈值（触发传播）
        ...
        with self._lock:
            ...
            for name, profile in self._skill_profiles.items():
                if profile.total_calls < 3:
                    continue  # 样本太少，不参与传播
                scores[name] = (
                    profile.avg_retention * 0.5 +
                    profile.success_rate * 0.3 +
                    profile.avg_compression * 0.2
                )
    ...
    def recommend_skill_order(self, difficulty: float) -> List[Tuple[str, float]]:
        ...
        with self._lock:
            ...
            for name, profile in self._skill_profiles.items():
                ...
                # 基于历史表现和历史难度匹配度计算得分
                history_score = (
                    profile.avg_retention * 0.4 +
                    profile.success_rate * 0.3 +
                    profile.avg_compression * 0.3
                )

                # 最近使用惩罚（鼓励探索）
                time_since_last = time.time() - profile.last_used
                recency_factor = min(time_since_last / 3600, 1.0) * 0.1
    ...
    def _learn_from_session(self, session_id: str,
        ...
        memory = N9R20SessionMemory(
            ...
            query_preview=output.output[:100],
            ...
            success=output.semantic_retention >= 0.85,
            skill_performances={
                "compression-core": output.semantic_retention,
                "dual-reasoner": output.semantic_retention * 0.9,
                "semantic-graph": output.semantic_retention * 0.85,
        ...
        # 定期传播（每 10 个会话）
        if self._skill_books["compression-core"].total_calls % 10 == 0:
        ...
        # 定期遗忘（每 50 个会话）
        if self._skill_books["compression-core"].total_calls % 50 == 0:
    ...
    def _handle_compression_complete(self, event: N9R20CompressionCompleteEvent) -> None:
        ...
        if state:
            routing = N9R20RoutingDecision(
                ...
                target_fold_depth=getattr(state, 'target_fold_depth', 4),
                target_compression_ratio=getattr(state, 'target_compression_ratio', 2.5),
