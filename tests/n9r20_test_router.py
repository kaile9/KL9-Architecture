"""
9R-2.0 RHIZOME · N9R20AdaptiveRouter 单元测试
...
from skills.n9r20_adaptive_router import N9R20AdaptiveRouter


class TestN9R20AdaptiveRouterSpecializedDetection(unittest.TestCase):
    """专用文本检测"""

    def setUp(self):
        self.n9r20_router = N9R20AdaptiveRouter()

    def test_buddhist_text_is_specialized(self):
        """佛教文本被识别为专用"""
        is_spec, conf = self.n9r20_router._detect_specialized_text(
    ...
    def test_short_greeting_is_not_specialized(self):
        """短问候不被识别为专用"""
        is_spec, conf = self.n9r20_router._detect_specialized_text("你好")
        self.assertFalse(is_spec)
...
class TestN9R20AdaptiveRouterDifficulty(unittest.TestCase):
    """难度评估"""
    ...
    def test_short_simple_text_low_difficulty(self):
        """短简单文本 → 低难度"""
        diff = self.n9r20_router._assess_difficulty("你好")
    ...
    def test_long_specialized_text_high_difficulty(self):
        ...
        diff = self.n9r20_router._assess_difficulty(long_query)
        # 长文本 + 专用术语 → difficulty 应高于短通用文本
        short_diff = self.n9r20_router._assess_difficulty("你好")
    ...
    def test_difficulty_range(self):
        ...
        for tc in test_cases:
            diff = self.n9r20_router._assess_difficulty(tc)
    ...
    def test_tension_keywords_increase_difficulty(self):
        """张力关键词提高难度"""
        without_tension = self.n9r20_router._assess_difficulty("这是一个简单的描述")
        with_tension = self.n9r20_router._assess_difficulty("但是这里有一个矛盾的问题")
        self.assertGreaterEqual(with_tension, without_tension * 0.8)


class TestN9R20AdaptiveRouterFoldBudget(unittest.TestCase):
    """fold 预算分配"""
    ...
    def test_fold_range_always_2_to_9(self):
        ...
        for length, diff, is_spec in test_cases:
            fold = self.n9r20_router._allocate_fold_budget(length, diff, is_spec)
    ...
    def test_simple_task_low_fold(self):
        """简单任务 → 低 fold"""
        fold = self.n9r20_router._allocate_fold_budget(50, 0.05, False)
        self.assertLessEqual(fold, 3)

    def test_hard_task_high_fold(self):
        """困难任务 → 高 fold"""
        fold = self.n9r20_router._allocate_fold_budget(500, 0.95, True)
        self.assertGreaterEqual(fold, 6)

    def test_long_text_increases_fold(self):
        """长文本增加 fold"""
        fold_short = self.n9r20_router._allocate_fold_budget(100, 0.5, False)
        fold_long = self.n9r20_router._allocate_fold_budget(1200, 0.5, False)
        self.assertGreaterEqual(fold_long, fold_short)

    def test_specialized_text_reduces_fold(self):
        """专用文本减少 fold（压缩更高效）"""
        fold_generic = self.n9r20_router._allocate_fold_budget(300, 0.6, False)
        fold_specialized = self.n9r20_router._allocate_fold_budget(300, 0.6, True)
        self.assertLessEqual(fold_specialized, fold_generic)


class TestN9R20AdaptiveRouterCompressionTarget(unittest.TestCase):
    """目标压缩率"""
    ...
    def test_target_ratio_in_range_2_to_2_5(self):
        ...
        for length, diff in test_cases:
            ratio = self.n9r20_router._compute_target_ratio(length, diff)
    ...
    def test_long_text_lower_ratio(self):
        """长文本 → 更宽松的压缩率（2.0）"""
        ratio = self.n9r20_router._compute_target_ratio(800, 0.5)
        self.assertAlmostEqual(ratio, 2.0, delta=0.1)

    def test_short_text_higher_ratio(self):
        """短文本 → 更激进的压缩率（2.5）"""
        ratio = self.n9r20_router._compute_target_ratio(100, 0.5)
        self.assertAlmostEqual(ratio, 2.5, delta=0.1)

    def test_high_difficulty_relaxes_ratio(self):
        """高难度放松压缩率"""
        ratio_easy = self.n9r20_router._compute_target_ratio(200, 0.3)
        ratio_hard = self.n9r20_router._compute_target_ratio(200, 0.9)
        self.assertGreaterEqual(ratio_easy, ratio_hard - 0.1)


class TestN9R20AdaptiveRouterUrgency(unittest.TestCase):
    """紧急度计算"""
    ...
    def test_urgency_range(self):
        ...
        for diff in [0.0, 0.3, 0.5, 0.8, 1.0]:
            for length in [10, 200, 1000, 3000]:
                urgency = self.n9r20_router._compute_urgency(diff, length)
                self.assertGreaterEqual(urgency, 0.0)
                self.assertLessEqual(urgency, 1.0)

    def test_high_difficulty_increases_urgency(self):
        """高难度 → 高紧急度"""
        low = self.n9r20_router._compute_urgency(0.2, 100)
        high = self.n9r20_router._compute_urgency(0.9, 100)
        self.assertGreater(high, low)

    def test_very_long_text_max_urgency(self):
        """极长文本 → 接近最大紧急度"""
        urgency = self.n9r20_router._compute_urgency(0.8, 3000)
        self.assertGreaterEqual(urgency, 0.7)


class TestN9R20AdaptiveRouterFullRoute(unittest.TestCase):
    """完整路由决策"""
    ...
    def test_route_returns_routing_decision(self):
        """route() 返回 N9R20RoutingDecision"""
        result = self.n9r20_router.route("量子纠缠与空性")
        self.assertIsInstance(result, N9R20RoutingDecision)

    def test_route_all_fields_populated(self):
        """route() 填充所有字段"""
        result = self.n9r20_router.route("一段中等长度的测试文本用于路由决策评估")
    ...
    def test_specialized_path_for_buddhist_text(self):
        ...
        result = self.n9r20_router.route(
    ...
    def test_standard_path_for_casual_text(self):
        """日常文本走 standard 路径"""
        result = self.n9r20_router.route("今天中午吃什么好呢")
    ...
    def test_route_consistency(self):
        ...
        r1 = self.n9r20_router.route(query)
        r2 = self.n9r20_router.route(query)
...
class TestN9R20AdaptiveRouterConceptDensity(unittest.TestCase):
    """概念密度检测"""
    ...
    def test_high_density_text(self):
        """高密度文本"""
        density = self.n9r20_router._compute_concept_density(
    ...
    def test_low_density_text(self):
        """低密度文本"""
        density = self.n9r20_router._compute_concept_density("hello world test")
        # 纯英文 → 无中文匹配 → 0
        self.assertEqual(density, 0.0)

    def test_empty_text_density(self):
        """空文本概念密度为 0"""
        self.assertEqual(self.n9r20_router._compute_concept_density(""), 0.0)
    ...
    def test_density_range(self):
        ...
        for tc in test_cases:
            d = self.n9r20_router._compute_concept_density(tc)
            self.assertGreaterEqual(d, 0.0)
            self.assertLessEqual(d, 1.0)


class TestN9R20AdaptiveRouterN9R20TensionDetection(unittest.TestCase):
    """张力关键词检测"""
    ...
    def test_tension_detected(self):
        """检测到张力"""
        tf = self.n9r20_router._detect_tension_keywords("但是这里有一个矛盾的问题")
        self.assertGreater(tf, 0.0)

    def test_no_tension(self):
        """无张力"""
        tf = self.n9r20_router._detect_tension_keywords("这是简单的描述")
        self.assertEqual(tf, 0.0)

    def test_multiple_tension_keywords(self):
        """多个张力关键词"""
        tf1 = self.n9r20_router._detect_tension_keywords("但是")
        tf2 = self.n9r20_router._detect_tension_keywords("但是矛盾冲突为什么如何")
    ...
    def test_tension_range(self):
        ...
        for tc in test_cases:
            tf = self.n9r20_router._detect_tension_keywords(tc)
            self.assertGreaterEqual(tf, 0.0)
            self.assertLessEqual(tf, 1.0)


class TestN9R20AdaptiveRouterEdgeCases(unittest.TestCase):
    """边界条件与极端输入"""
    ...
    def test_single_character(self):
        """单字符"""
        result = self.n9r20_router.route("空")
        self.assertIsInstance(result, N9R20RoutingDecision)
    ...
    def test_unicode_handling(self):
        """Unicode 处理"""
        result = self.n9r20_router.route("存在 être être 存在")
        self.assertIsInstance(result, N9R20RoutingDecision)

    def test_very_long_text(self):
        """超长文本（5000+ 字符）"""
        long_text = "量子纠缠与缘起性空的哲学对话。" * 200
        result = self.n9r20_router.route(long_text)
