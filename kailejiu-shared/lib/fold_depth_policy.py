...
def determine_max_fold_depth(query: str, tension_type: str = "", base_depth: int = 2, mode: str = "default") -> int:
    """根据 query 特性动态确定最大折叠深度。

    影响因素:
        - 张力类型的认知负载基数
        - query 长度（长 query 通常含更多维度）
        - 翻译模式固定 depth=2（术语级折叠无需深层递归）

    返回: [1, 5] 范围内的整数。
    """
    # 翻译模式：术语级折叠仅需2层（试译→对比→悬置），句子/段落级也够用
    if mode == "translation" or tension_type == "foreignizing_vs_domesticating":
        return 2
